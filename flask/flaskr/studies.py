...
def patients():
    ...
    query = "SELECT name, progress FROM Patients RIGHT JOIN Study1 ON patients.key_id=study1.patient"
    query2 = "SELECT name, progress FROM Patients RIGHT JOIN Study2 ON patients.key_id=study2.patient"
    cur.execute(query)
    result = cur.fetchall()
    cur.execute(query2)
